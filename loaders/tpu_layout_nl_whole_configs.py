...
class LayoutCollator:
    ...
    def __call__(self, batch : List, selected_configs:List[int]=None):
        ...
        loader = NeighborLoader(
            data,
            num_neighbors=[-1] * 1, # Sample all neighbors for each node for 1 iterations
            batch_size=4096, # Use a batch size for sampling training nodes
        ...
        for batch in loader:
            ...
            for idx in range(batch.n_id.shape[0]):
                assert torch.equal(batch.node_feat[idx], data.node_feat[batch.n_id[idx]]), ""
                assert torch.equal(batch.node_config_feat[idx], data.node_config_feat[batch.n_id[idx]]), ""

            # verify edges equivalance
        ...
        #TODO: check whether graph is connected after neighbor sampling or not

        batch_train_list = []
        for graph in batch_list:
            num_configs = graph.node_config_feat.shape[1]  #  graph.node_config_feat.shape = (num_nodes, num_configs, CONFIG_FEAT)

            node_config_ids   = graph.node_config_ids
            config_edge_index = graph.edge_index 
            node_feat         = graph.node_feat         # (num_nodes, (NODE_FEATS + 1)(141))
            ...
            for config_idx in range(num_configs):
                ...
                config_graph.validate(raise_on_error=True)
                assert config_graph.is_directed(), ""
...
class TPULayoutDatasetFullConfigs(torch.utils.data.Dataset):
...
if __name__ == '__main__':
    dataset = TPULayoutDatasetFullConfigs(data_dir="/home/cc/data/tpugraphs/npz", 
